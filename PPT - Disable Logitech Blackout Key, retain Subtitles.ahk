...
SetTimer SubtitlesCheck, 10000   ;Sets a timer to run the SubtitlesCheck sub every 10 seconds

SubtitlesCheck() {

 
 if WinExist('ahk_exe POWERPNT.EXE ahk_class screenClass') {
        WinGetPos &Xpos, &Ypos ,&Width, &Height ; Use the window found by WinExist.

        if (  
           
           (  PixelGetColor(Xpos+20, Ypos+ 100 ) == "0x000000" )  &&
           (  PixelGetColor((Xpos+Width)-20, Ypos+ 100 ) == "0x000000" ) &&
           (  PixelGetColor(Xpos+20, (Ypos+Height)- 100) == "0x000000" ) &&
           (  PixelGetColor((Xpos+Width)-20, (Ypos+Height)-100) == "0x000000" )
           
           ) {
            OutputDebug "ahk:detected powerpoint subtitles border"
        } else {
           
         
            ; the black border around the presentation has dissapeared - restart subtitles by sending the J key
                  ...
                  if WinExist('ahk_exe POWERPNT.EXE ahk_class screenClass') {
                     WinGetPos &Xpos, &Ypos ,&Width, &Height ; Use the window found by WinExist.
                            if (  
                                    ...
                                    OutputDebug "detected after activate"
                                    ; activating it resolved the issue.
                                
                                } else {
                
                                        if WinExist('ahk_exe POWERPNT.EXE ahk_class PodiumParent') {
                                                OutputDebug "ahk:restarting via PodiumParent"
                                                WinActivate ; Use the window found by WinExist.
                                                SendCustom 1, "j"
                                            } else {
                                                if WinExist('ahk_exe POWERPNT.EXE ahk_class screenClass') {
                                                    OutputDebug "ahk:restarting via PPT screenClass"
                                                    WinActivate ; Use the window found by WinExist.
                                                }
                                                Send "j"
                                            }
        ...
        OutputDebug "ahk:Powerpoint not running"
    }
